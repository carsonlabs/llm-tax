...
function extractMethod(line: string): string {
  ...
  if (lower.includes(".post(") || lower.includes("method: 'post'") || lower.includes('method: "post"')) return "POST";
  if (lower.includes(".put(") || lower.includes("method: 'put'") || lower.includes('method: "put"')) return "PUT";
  if (lower.includes(".patch(") || lower.includes("method: 'patch'") || lower.includes('method: "patch"')) return "PATCH";
  if (lower.includes(".delete(") || lower.includes("method: 'delete'") || lower.includes('method: "delete"')) return "DELETE";
  return "GET";
}
...
export async function scanCodebase(dir: string): Promise<DiscoveredCall[]> {
