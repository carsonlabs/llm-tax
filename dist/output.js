...
function wasteBar(percent, width = 20) {
...
export function printReport(report) {
    ...
    console.log(`  ${c.bold}${report.method}${c.reset} ${urlStr}`);
    console.log(`  ${c.dim}${report.file}:${report.line}${c.reset}`);
    console.log(`  ${bar} ${wasteColor(report.wastePercent)}${c.bold}${waste} waste${c.reset}  ${c.dim}│${c.reset}  ${report.totalTokens} tokens → ${report.usedTokens} used, ${c.red}${report.wastedTokens} wasted${c.reset}`);
    console.log(`  ${c.dim}Fields: ${report.totalFields} total, ${report.usedFieldCount} used  │  ${report.responseTimeMs}ms  │  HTTP ${report.statusCode}${c.reset}`);
    console.log();
}
export function printSummary(summary) {
    ...
    for (const model of ["gpt-4o", "gpt-4o-mini", "claude-sonnet", "claude-haiku"]) {
        const total = monthlyCost[model];
        const waste = monthlyWaste[model];
        const pct = total > 0 ? Math.round((waste / total) * 100) : 0;
        console.log(`  ${c.bold}${model.padEnd(21)}${c.reset}${formatDollars(total).padEnd(16)}${c.red}${formatDollars(waste).padEnd(16)}${c.reset}${wasteColor(pct)}${pct}%${c.reset}`);
    }
    console.log();
...
export function printUnreachable(reports) {
